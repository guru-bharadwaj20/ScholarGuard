...
> **Re-ordered after Stage 2h.** The old list was about making detectors better. The measurement now says the binding constraint is one level up: per-figure recall and false-alarm intervals overlap for *every* detector, so any paper-level aggregate is combining signals that are not individually separated yet. Items 1 and 2 are therefore new; the previous list follows as items 3-8.

1. **Establish that a detector separates at all, before tuning any aggregate.** In all nine measurable recall/FPR cells the two intervals overlap. Copy-move on set 1 (0.57 [0.43–0.69] vs 0.39 [0.35–0.43]) is the closest to a clean separation and needs roughly a 3× larger positive set to resolve — 51 annotated positive figures is the current denominator. More annotated fraud figures is worth more than any new detector: it is the only thing that makes "did this change help?" answerable at the figure level, where the signal actually lives.
2. **Stop optimising the paper-level AUC.** Pooled and count-controlled it is 0.610 [0.482, 0.725]. With three sets already spent, the noise floor on that number is wider than any aggregation change realistically moves it — which is exactly what Stage 2h's six combiners demonstrated. Paper-level work should now target *calibration* (where the conformal cutoff gives a guarantee rather than an estimate) rather than *ranking*.

3. **Annotate manipulation *type*, not just location.** Copy-move and splice now have recall; cross-figure and AI still do not, purely because each annotated figure inherits one `fraud_type` from its paper's retraction reason and no reason mapped to those modes. The notices already describe the manipulation ("duplicated from", "spliced", "overlapping with Figure X in another paper") — classifying that phrase per figure would complete the picture for every detector. This is the highest-value next step: it is the only thing that makes cross-figure and AI improvements steerable at all.
4. **Fix cross-figure specificity — but not with the residual test.** It remains the largest fixable false-positive source (27–29% per-figure), and Stage 2g rules out the approach the roadmap previously assumed: the matches pass the noise test rather than failing it. What is left is semantic — a caption/panel-role check so a labelled dose-response or time series is not read as duplication — plus a same-package duplicate-file guard, since 4 of 50 clean packages ship the same image under two names.
5. **Make splice work, or retire it.** Recall 0.02 on all three sets means it is close to inert. Its 0.8% FPR bought nothing, because it almost never fires on the figures that matter. Either loosen it now that recall is measurable and the trade-off is visible, or drop it and reclaim its 20 risk-score points.
6. **Improve copy-move precision.** It is the one detector with proven sensitivity (recall ~0.5) and it produces the bulk of the false alarms (precision ~0.15). PatchMatch verification and a Zernike-moment rotation-invariant CMFD tier target the subtle cases SIFT misses; with recall now measurable, both sides of any change are finally visible.
7. **Keep multi-set validation as the standard, and budget for fresh sets.** Stage 2g showed a single 30/50 held-out set can reverse an 0.05 AUC verdict; Stage 2h needed a third set because sets 1 and 2 had both become calibration data. All three are now spent. A full evaluation is ~3 minutes at `--workers 8`, so re-running is free — what is not free is an *unseen* set, and every accepted change consumes one. Fetching set 4 before the next change, not after, is part of the change.
